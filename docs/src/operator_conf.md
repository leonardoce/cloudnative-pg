...
`ENABLE_POD_PATCH_ANNOTATION` | When set to `true`, enables the `cnpg.io/podPatch` annotation feature. **WARNING: This can be used as a privilege escalation vector** - users with Cluster CRD permissions can create arbitrary Pod configurations using the operator privileges. Only enable in environments where all Cluster CRD users are fully trusted. Default is `false`. See [Security-sensitive options](#security-sensitive-options) for more details.
...
Some operator configuration options have security implications that require
careful consideration before enabling.

### Pod Patch Annotation (`ENABLE_POD_PATCH_ANNOTATION`)

The `cnpg.io/podPatch` annotation allows users to apply arbitrary JSON patches
to the Pod specifications generated by the operator. While this provides
flexibility for advanced customization, it introduces a privilege escalation
risk.

Only enable this feature in environments where all users with Cluster CRD
