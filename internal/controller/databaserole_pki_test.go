...
var _ = Describe("databaserole_pki", func() {
	...
	Describe("issueClientCertificate", func() {
		...
		It("keeps the cert Secret and reports expiration when cert is still valid (existing-secret path)", func(ctx SpecContext) {
			_, _ = generateFakeCASecret(r.Client, cluster.GetClientCASecretName(), namespace, "test.example.com")
	...
	Describe("deleteOwnedCertSecret", func() {
		It("deletes the owned cert Secret and clears status when issueClientCertificate is set to false", func(ctx SpecContext) {
			_, _ = generateFakeCASecret(r.Client, cluster.GetClientCASecretName(), namespace, "test.example.com")
